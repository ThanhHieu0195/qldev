        ...
        switch ($group) {
            case G_TASK : // Quản lý công việc
                if (verify_access_right ( $account, G_TASK, KEY_GROUP )) {
                    ...
                    $content = "<p>
                                ...
                                <small><a href='../task/task-list.php' class='remove-link' title='Danh sách công việc'>Xem chi tiết</a></small>
                            </p>";
                }
                break;
                ...
                if (verify_access_right ( $account, F_EQUIPMENT_ASSIGNED_LIST )) {
                    ...
                    if ($count != 0) {
                        $content = "<p>
                                    ...
                                    <small><a href='../equipment/re-assign-list.php' class='remove-link' title='Dụng cụ mới nhận'>Xem chi tiết</a></small>
                                </p>";
                    } else {
                        $content = "<p>
                                    Không có dụng cụ mới được phân công.
                                    <br />Xem danh sách các dụng cụ do bạn chịu trách nhiệm <small><a href='../equipment/assigned-list.php' class='remove-link' title='Danh sách dụng cụ giao cho bạn'>tại đây</a></small>.
                                </p>";
                    }
                }
                break;
                    ...
                    if (verify_access_right ( $account, F_ORDERS_RESERVATION_LIST )) {
                        ...
                        if ($obj->total > 0) {
                            $t = "<p>
                                        ...
                                        <small><a href='../orders/reservationlist.php' class='remove-link' title='Duyệt thông tin đơn hàng'>Xem chi tiết</a></small>
                                    </p>";
                        } else {
                            $t = "<p>
                    ...
                    if (verify_access_right ( $account, F_ORDERS_SPECIAL_LIST )) {
                        ...
                        if ($obj->total > 0) {
                            $z = "<p>
                                    ...
                                    <small><a href='../orders/special_list.php' class='remove-link' title='Đơn hàng cần theo dõi đặc biệt'>Xem chi tiết</a></small>
                                </p>";
                        } else {
                            $z = "<p>
                ...
                if (verify_access_right ( $account, F_ITEMS_UPLOAD )) {
                    ...
                    if ($obj->files > 0) {
                        $t = "<p>
                                        ...
                                        <small><a href='../items/upload.php' class='remove-link' title='Quản lý upload sản phẩm'>Chi tiết</a></small>
                                    </p>";
                    } else {
                        $t = "<p>
                ...
                if (verify_access_right ( $account, F_STORES_SWAP )) {
                    ...
                    if ($obj->total > 0) {
                        $z = "<p>
                                ...
                                <small><a href='../stores/ingoing-list.php' class='remove-link' title='Danh sách phiếu chuyển hàng đến'>Chi tiết</a></small>
                              </p>";
                    } else {
                        $z = "<p>
            ...
            case G_REWARDS_PENALTY : // Ghi nhận và đóng góp
                if (verify_access_right ( $account, G_REWARDS_PENALTY, KEY_GROUP )) {
                    ...
                    // Created link
                    if ($obj->created > 0) {
                        $t .= "<small><a href='../rewards_penalty/created-list.php?top_days={$top_days}' class='remove-link' title='Ghi nhận bạn đã tạo'>Xem</a></small>";
                    }
                    $t .= "<br />
                            &nbsp;&nbsp;<b>•</b> <span style='color: blueviolet'><b>{$obj->assigned}</b></span> ghi nhận/đóng góp về bạn";
                    // Assigned link
                    if ($obj->assigned > 0) {
                        $t .= "<small><a href='../rewards_penalty/assigned-list.php?top_days={$top_days}' class='remove-link' title='Ghi nhận về bạn'> Xem</a></small>";
                    }
                    $t .= "</p>";
                    ...
                    if (verify_access_right ( $account, F_REWARDS_PENALTY_UNAPPROVED_LIST )) {
                        ...
                        if ($count > 0) {
                            $z = "<p>
                                    ...
                                    <small><a href='../rewards_penalty/unapproved-list.php' class='remove-link' title='Approve ghi nhận/đóng góp'>Approve</a></small>
                                   </p>";
                        } else {
                            $z = "<p>
            ...
            case G_WORKING_CALENDAR : // Quản lý lịch làm việc
                ...
                if (verify_access_right ( $account, F_WORKING_CALENDAR_CALENDAR )) {
                    $z = "<p>
                            ...
                            Xem lịch làm việc mới nhất <small><a href='../working_calendar/calendar.php' class='remove-link' title='Lịch làm việc'>tại đây</a></small>
                          </p>";
                }
                ...
                if (verify_access_right ( $account, array (
                    ...
                    if ($obj ['plan'] > 0 || $obj ['leave_days_add'] > 0 || $obj ['leave_days_change'] > 0) {
                        ...
                        if ($obj ['plan'] > 0 && verify_access_right ( $account, F_WORKING_CALENDAR_APPROVE_CALENDAR )) {
                            $t .= "&nbsp;&nbsp;<b>•</b> <span style='color: blue'><b>{$obj['plan']}</b></span> lịch làm việc được tạo mới <small><a href='../working_calendar/approve-calendar.php' class='remove-link' title='Approve lịch làm việc'>Approve</a></small><br />";
                        }
                        // Approve leave days
                        if ($obj ['leave_days_add'] > 0 && verify_access_right ( $account, F_WORKING_CALENDAR_APPROVE_LEAVE_DAYS_ADD )) {
                            $t .= "&nbsp;&nbsp;<b>•</b> <span style='color: #ff6600'><b>{$obj['leave_days_add']}</b></span> request xin nghỉ thêm <small><a href='../working_calendar/approve-leave-days-add.php' class='remove-link' title='Approve xin nghỉ thêm'>Approve</a></small><br />";
                        }
                        ...
                        if ($obj ['leave_days_change'] > 0 && verify_access_right ( $account, F_WORKING_CALENDAR_APPROVE_LEAVE_DAYS_CHANGE )) {
                            $t .= "&nbsp;&nbsp;<b>•</b> <span style='color: blueviolet'><b>{$obj['leave_days_change']}</b></span> request dời ngày nghỉ <small><a href='../working_calendar/approve-leave-days-change.php' class='remove-link' title='Approve dời ngày nghỉ'>Approve</a></small><br />";
                        }
                        $t .= "</p>";
                    } else {
                        $t = "<p>
            ...
            case G_GUEST_DEVELOPMENT : // Phát triển khách hàng
                if (verify_access_right ( $account, G_GUEST_DEVELOPMENT, KEY_GROUP )) {
                    ...
                    $content .= "<p>
                                    <img src='../resources/images/icons/calendar_16.png' alt='calendar'>
                                    Xem lịch sự kiện - liên hệ khách hàng <small><a href='../guest_development/calendar.php' class='remove-link' title='Lịch sự kiện - liên hệ khách hàng'>tại đây</a></small>
                                </p>";
                }
                break;
        }
