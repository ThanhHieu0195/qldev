<?php
    ...
    if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
    {
        ...
        $sWhere .= ')';
        $sWhere .= sprintf(" AND (trangthai = '%s') AND (checked = '0') AND (approved = '%s')", donhang::$DA_GIAO, donhang::$APPROVED);
    }
    ...
    if ( $sWhere == "" )
        $sWhere = sprintf("WHERE (trangthai = '%s') AND (checked = '0') AND (approved = '%s')", donhang::$DA_GIAO, donhang::$APPROVED);
    ...
    while ( $aRow = mysql_fetch_array( $rResult ) )
    {
        ...
        // Dien thoai
        $fmt = "&bull; <span class='blue'>%s</span><br />";
        $g = $khach->detail_by_id($aRow[ 'sanpham' ]);
        $dt = "";
        $dt .= ($g->dienthoai1 != "") ? sprintf($fmt, $g->dienthoai1) : "";
        $dt .= ($g->dienthoai2 != "") ? sprintf($fmt, $g->dienthoai2) : "";
        $dt .= ($g->dienthoai3 != "") ? sprintf($fmt, $g->dienthoai3) : "";
        $row[] = $dt;
