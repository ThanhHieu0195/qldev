 <?php 
    ...
    foreach ($data_detail as $obj):
	    ...
	    $row_soluongdutoan = "<input class='soluongdutoan' value='%s' type='text' placeholder='0' onkeyup='autoExpectNum($id_category, %s)' %s>";
	    ...
	    $fm = "<tr idrow='%s' class='alt-row'> 
		    ...
		    <td>%s</td>
		    <td>%s %s</td></tr>";
	    $data_material_category[$id_category]['fm'] = sprintf($fm, '{0}', sprintf($row_name_material_category, '{0}'), sprintf($row_dongiavattu, '0'), sprintf($row_soluongdutoan, '0', '{0}', 'readonly'), 
                                                                  sprintf($row_giadutoan, 0), sprintf($row_add_row, '{0}'), sprintf($row_remove_row_html, '{0}'));

	    $detail_material_category = new detail_material_category();
	    $list_detail_material_category = $detail_material_category->getDetail( array('idcongtrinh' => $id_building, 'idhangmuc' => $id_category), array('idvattu', 'id','soluongdutoan','dongiavattu') );
			...
			<div id="container">
				...
				<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" idcategory="<?php echo $id_category; ?>">
					...
					<?php 
						...
						$giadutoan = $obj['soluongdutoan']*$obj['dongiavattu'];
                                                 
						echo sprintf( $fm, $idrow, sprintf($row_name_material_category, $idrow), sprintf($row_dongiavattu, $dongiavattu), 
                                                             sprintf( $row_soluongdutoan, $obj['soluongdutoan'], $idrow, ''), sprintf( $row_giadutoan, number_2_string($giadutoan,'.')), 
