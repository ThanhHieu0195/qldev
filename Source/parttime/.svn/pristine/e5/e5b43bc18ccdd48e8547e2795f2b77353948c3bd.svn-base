<?php
	 ...
	 */
	$aColumns = array( 'tranh.masotranh', 'tranh.tentranh', 'loaitranh.tenloai', 'tranh.giaban', 'khohang.tenkho', 'tonkho.soluong', 'tranh.hinhanh', 'tonkho.makho', 'tranh.loai' );
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			else if ( $column_name != ' ' )
			{
                ...
                if ( $column_name == "masotranh" )
                {
                    $thongke_soluong = $tranh->statistic_number($aRow[ $column_name ]);
                }
			}
		}
        $row[] = $thongke_soluong;
