<?php
    ...
    $name_list = array( 'equipment_id',  //0
                       'equipment_id',  //1
                       'name',          //2
                       'status',        //3
                       'tenkho',        //4: stored_in
    ...
    $sTable = 'equipment INNER JOIN khohang ON equipment.stored_in = khohang.makho 
                         INNER JOIN nhanvien ON equipment.assign_to = nhanvien.manv';
    $sGroupBy = '';
    ...
    if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
    {
        ...
        {
            if($i < 4) // index of searching columns
            {
                $column = $aColumns[$i];
    ...
    // assign_to
    $assign_to = "";
    if(isset($_GET['assign_to']) && $_GET['assign_to'] != "")
    ...
    //$enable = BIT_TRUE;
    //$additional = "(enable = {$enable})";
    $additional = "";
    
    if($stored_in != -1 && $assign_to != "") {
        $additional .= " (stored_in = {$stored_in}) AND (assign_to = '{$assign_to}')";
    } else {
        if($stored_in != -1)
            $additional .= " (stored_in = {$stored_in})";
        else if($assign_to != "")
            $additional .= " (assign_to = '{$assign_to}')";
    }
    //debug($stored_in);
    //debug($assign_to);
