...
function editrow(id_row) {
	var row = list_group_construction[id_row];
	if (row) {
		console.log(row);
		$('#fedit_id').val(row[0]);
		$('#fedit_title').html(row[0]);
		$('#fedit_name_group').val(row[1]);
		$('#fedit_address').val(row[2]);
		$('#fedit_num_phone').val(row[3]);
		$('#fedit_id_group').val(row[4]);
                $('#fedit_nangluc').val(row[7]);
		show_popup_edit();
	}
};
