<?php
	 ...
	 */
	$aColumns = array('vthmtc.id', 'idhangmuc', 'tenhangmuc', 'tenvattu', 'thongso', 'giathap', 'giacao', 'donvitinh');
	$bcolumns = array('id', 'idhangmuc', 'tenvattu', 'thongso', 'giathap', 'giacao', 'donvitinh');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "vthmtc.idhangmuc";
	
	/* DB table to use */
	$sTable = "vattuhangmucthicong vthmtc join hangmucthicong hm on vthmtc.idhangmuc = hm.id ";
	...
	while ( $row = mysql_fetch_array( $rResult ) )
	{
		...
		for ($i=0; $i < count($bcolumns); $i++) { 
			...
			switch ($ncolumn) {
				case 'idhangmuc':
					$arow[] = array($row['idhangmuc'], $row['tenhangmuc']);
					break;
				case 'donvitinh':
					$arow[] = array($row[$ncolumn], material_category::change_unit($row[$ncolumn]));
					break;
				case 'giathap':
				case 'giacao':
					$arow[] = number_2_string($row[$ncolumn]);
					break;
				default:
					$arow[] = $row[$ncolumn];
					break;
