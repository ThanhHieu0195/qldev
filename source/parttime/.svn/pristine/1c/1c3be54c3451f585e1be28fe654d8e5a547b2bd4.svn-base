    ...
    try {
        ...
        if (isset($add_new)) {
            if (verify_access_right ( current_account (), F_GUEST_DEVELOPMENT_ADD_NEW )) {
                // Get input data
                //error_log ("Add new", 3, '/var/log/phpdebug.log');
                $makhach = $_POST ['guest_id'];
                $hoten = $_POST ['hoten'];
                $diachi = $_POST ['diachi'];
                ...
                $guest->makhach = $makhach;
                $guest->hoten = $hoten;
                $guest->manhom = $nhomkhach;
                $guest->diachi = $diachi;
                $guest->dienthoai1 = $dienthoai1;
                $guest->dienthoai2 = $dienthoai2;
                $guest->dienthoai3 = $dienthoai3;
                $guest->email = $email;
                $guest->emailphu = $emailphu;
                $today = current_timestamp('Y-m-d');
                if ((isset($makhach)) && (! empty($makhach))) {
                    if (! $khach_model->update ( $guest )) { $continue = FALSE; }
                } else {
                    if (! $khach_model->insert ( $guest )) { $continue = FALSE; }
                }
                if ($continue) {
                    ...
                    if (isset ( $_POST ['add_new_buy'] )) {
                        $_SESSION["ordernumber"] = $guest->makhach;
                    }
                    ...
                    if ($responsibility_model->insert ( $res )) {
                        ...
                        $event->note = "Thêm khách vào development";
                        $event->is_event = BIT_FALSE;
                        $event->enable = BIT_TRUE;

                        if (! $events_model->is_existing_schedule($guest_id, $today)) {
                        ...
                        if ($continue)
                        {
                            ...
                            $event->note = "Hẹn lịch liên hệ lại"; // Default note
                            $event->is_event = BIT_FALSE;
                            $event->enable = BIT_TRUE;
                            if (! $events_model->is_existing_schedule($guest_id, $next_schedule)) {
                            ...
                            if ($continue)
                            {
                                ...
                                if (count ( $events ) > 0) {
                                    foreach ( $events as $z ) {
                                        ...
                                        // Insert to database
                                        if (! $events_model->is_existing_schedule($guest->makhach, $z ['day'])) {
                                            $continue =  $events_model->insert ( $item );
                                        }
                                        if ($continue) {
                                            // Do nothing
                                        } else {
                            ...
                            {
                                $result ['message'] = "Lỗi thêm ngày liên hệ: '{$events_model->getMessage()}'";
                            }
                        }
                        else
                        {
                            $result ['message'] = "Lỗi thêm nội dung liên hệ: '{$history_model->getMessage()}'";
                        }
                    } else {
        ...
        // Add new from db
        if (isset($add_from_db)) {
            error_log ("add from db", 3, '/var/log/phpdebug.log');
        ...
        if (isset ( $_POST ['edit'] )) {
            // Get input data
            error_log ("Edit", 3, '/var/log/phpdebug.log');
            $guest_id = $_POST ['guest_id'];
            $hoten = $_POST ['hoten'];
            $diachi = $_POST ['diachi'];
            $quan = $_POST ['quan'];
        ...
        if (isset ( $_POST ['list_all_action'] ) && $_POST ['list_all_action'] == "add_favourites") {
            if (verify_access_right ( current_account (), F_GUEST_DEVELOPMENT_LIST_FAVOURITE )) {
                // Get input data
                error_log ("Favourist", 3, '/var/log/phpdebug.log');
                $guest = $_POST ['guest'];
        ...
        // Add guests to favourites
        if (isset ( $_POST ['list_all_action'] ) && $_POST ['list_all_action'] == "unfollow") {
            if (verify_access_right ( current_account (), F_GUEST_DEVELOPMENT_ADD_NEW )) {
                // Get input data
                $khach_model = new khach ();
                $guest = $_POST ['guest'];
                ...
                if (count ( $guest ) > 0) {
                    ...
                    // Output result
                    $result ['message'] = "Them thanh cong {$success}/{$total} khach hang vao pool reassign";
                    if (count ( $detail ) == 0) {
                        $result ['result'] = "success";
                    } else {
                        $result ['message'] .= "Có một số lỗi như sau: ";
        ...
        if (isset ( $_POST ['list_all_action'] ) && $_POST ['list_all_action'] == "re_assign") {
            if (verify_access_right ( current_account (), F_GUEST_DEVELOPMENT_LIST_ALL )) {
                // Get input data
                error_log ("Responsibility", 3, '/var/log/phpdebug.log');
                $guest = $_POST ['guest'];
        ...
        if (isset ( $_POST ['remove_favourite'] )) {
            if (verify_access_right ( current_account (), F_GUEST_DEVELOPMENT_LIST_FAVOURITE )) {
                // Get input data
                 error_log ("Remove favourist", 3, '/var/log/phpdebug.log');
                $guest_id = $_POST ['guest_id'];
                $employee_id = current_account ();
        ...
        if (isset ( $_POST ['statistic_updated'] )) {
            if (verify_access_right ( current_account (), F_GUEST_DEVELOPMENT_STATISTIC_UPDATED )) {
                // Get input data
                error_log ("Statistic update", 3, '/var/log/phpdebug.log');
        ...
        if (isset ( $_POST ['statistic_contacted'] )) {
            if (verify_access_right ( current_account (), F_GUEST_DEVELOPMENT_STATISTIC_CONTACTED )) {
                // Get input data
                error_log ("Statistic contacted", 3, '/var/log/phpdebug.log');
                $from_date = $_POST ['from_date'];
                $to_date = $_POST ['to_date'];
        ...
        if (isset ( $_POST ['contact'] )) {
            error_log ("Contact", 3, '/var/log/phpdebug.log');
            // Initial values
            $contact_content = "";
            $next_schedule = "";
            $note = "";
            ...
            // Validate data
            $valid = TRUE;
            if (empty($contact_content) && $is_cancelled == FALSE) // Noi dung lien he
            {
                $result ['message'] = "Nội dung liên hệ/ghi chú không được để trống!";
                $valid = FALSE;
            }
            if ($valid)
            {
                if ($is_cancelled == FALSE)
                {
                    ...
                    {
                        $result ['message'] = "Vui lòng chọn ngày liên hệ tiếp theo!";
                        $valid = FALSE;
                    }
                }
            }
            if ($valid)
            {
                if ($is_cancelled == FALSE)
                {
                    if (custom_day_diff($last_schedule_date, $next_schedule) == 0)
                    {
                        $result ['message'] = "Ngày liên hệ tiếp theo không được trùng với lịch hẹn liên hệ đã có trong hệ thống!";
                        $valid = FALSE;
                    }
                }
            }
                    ...
                    if ($guest != NULL) {
                        ...
                        if (! empty ( $contact_content )) {
                            if (! $responsibility_model->check_res_exists($guest_id)) {
                                ...
                                if ($khach_model->get_development($guest_id)==3){
                                    if (! $responsibility_model->re_assign($guest_id, current_account ())) {
                                        $continue = FALSE;
                                    }
                                }
                            }
                            $tmp = $events_model->schedule_from_date_history($guest_id,$today);
                            $event = new guest_events_entity ();
                            $event->guest_id = $guest_id;
                            $event->event_date = $today;
                            $event->note = $note;
                        ...
                        if ($continue) {
                            ...
                            $event->is_event = BIT_FALSE;
                            $event->enable = BIT_TRUE;
                            //error_log ($next_schedule, 3, '/var/log/phpdebug.log');

                            if (! empty ( $next_schedule )) {
                                $tmp = $events_model->schedule_from_date($guest_id, $today);
                                if ($tmp != NULL)
                                {
                                    // Update existing event
                                    $event->uid = $tmp['uid'];
                                    $continue = $events_model->update ( $event );
                                    //error_log ("Update schedule ", 3, '/var/log/phpdebug.log');
                                }
                                else
                                {
                                    // Insert next schedule of contacting
                                    ...
                                    //error_log ("insert_schedule", 3, '/var/log/phpdebug.log');
                                }
                            }
                            ...
                            if ($continue) {
                                if ($is_cancelled) {
                                    ...
                                    if ($khach_model->set_development_status ( $guest_id, GUEST_DEVELOPMENT_ONGOING )) {
                                        $date = current_timestamp('Y-m-d');
                                        $khach_model->set_development_date($guest_id, $date);
                                        $marketing_model = new marketing();
                                        if ($marketing_model->khach_exist($guest_id))
                                            $marketing_model->set_lienhe($guest_id);
                                        $result ['result'] = "success";
                                        $result ['message'] = "Thực hiện thao tác thành công";
                                        $result ['reload'] = 1; // Reload data of the page
                                    }
        ...
        if (isset ( $_POST ['change_contact_schedule'] )) {
            // Get input data
            error_log ("Change schedult", 3, '/var/log/phpdebug.log');
            $guest_id = $_POST ['guest_id'];
            $last_schedule_date = $_POST ['last_schedule_date'];
            $event_id = $_POST ['last_schedule_id'];
            $next_schedule = $_POST ['next_schedule'];
            ...
            if (empty($next_schedule))
            {
                $result ['message'] = "Vui lòng chọn ngày hẹn mới!";
                $valid = FALSE;
            }
            if ($valid)
            {
                if (!empty($last_schedule_date) && (custom_day_diff($last_schedule_date, $next_schedule) == 0))
                {
                    $result ['message'] = "Ngày liên hệ tiếp theo không được trùng với lịch hẹn liên hệ đã có trong hệ thống!";
                    $valid = FALSE;
                }
            ...
            {
                $history_model = new guest_development_history ();
                if ($history_model->check_updating(current_account(), $guest_id, $next_schedule))
                {
                    $result ['message'] = "Ngày liên hệ tiếp theo không được trùng với lịch hẹn liên hệ đã có trong hệ thống!";
                    $valid = FALSE;
                }
            ...
            else 
            {
                ...
                //if ($responsibility_model->check_responsibility ( current_account (), $guest_id )) {
                    ...
                    if ($guest != NULL) {
                        ...
                        if ($continue) {
                            $event = $events_model->detail($event_id);
                            $note = "Hẹn lịch liên hệ lại"; // Default note
                            if ($event != NULL)
                            {
                                 // Update existing event
                            ...
                            if ($continue) {
                                ...
                                $result ['message'] = "Lỗi cập nhật ngày liên hệ: '{$events_model->getMessage()}'";
                            }
                        }
                    }
                    else {
...
if (verify_access_right ( current_account (), F_GUEST_GUEST_LIST )) {
    try {
        // Show orders list of a guest
        if (isset ( $_POST ['show_orders_by_guest'] )) {
            if (verify_access_right ( current_account (), F_GUEST_GUEST_LIST )) {
                // Get input data
                $guest_id = $_POST ['guest_id'];
