...
class chitietsanphammapping extends database {
    public $_NAMETABLE = 'chitietsanphammapping';

    public $_MASOTRANH = 'masotranh';
    public $_MACHITIET = "machitiet";
    public $_SOLUONG = "soluong";
    public $_TOTAL_COLUMN = "3";
    public  function laymachitiet($masotranh) {
        $params = array('machitiet', 'soluong');
        $conditions = array('masotranh' => $masotranh);
        $arr = $this->getAll($params, '', $conditions);
        if (count($arr) > 0) {
            return $arr;
        }
        return null;
    ...
    public function insert( $params )
    {
        ...
        $sql = "insert into $this->_NAMETABLE($this->_MASOTRANH, $this->_MACHITIET, $this->_SOLUONG) values('$val');";
        $this->setQuery( $sql );
        $result = $this->query();
        if ( $result ) {
            return true;
