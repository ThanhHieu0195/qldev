<?php
    ...
    if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
    {
        ...
        {
            if($i != 2 && $i != 5) // IFNULL((SELECT thanhtien FROM donhang WHERE madon = bill_code), 0) AS money
            {
                $column = $aColumns[$i];
    ...
    switch ($type) {
        case 'assign':
            $status = COUPON_STATUS_ASSIGN;
            $additional .= " AND (status = '$status') AND(DATEDIFF(coupon_assign.expire_date, CURRENT_TIMESTAMP) >= 0)";
            break;
