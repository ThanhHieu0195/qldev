<?php
    ...
    /* DB table to use */
    $sTable = 'rewards_penalty INNER JOIN nhanvien ON rewards_penalty.assign_to = nhanvien.manv';
    $sGroupBy = '';
    ...
    $additional = "(approved = $approved) AND (created_by = '$created_by')";
    if ($top_days > 0) {
        $additional .= " AND (DATEDIFF(CURRENT_TIMESTAMP(), created_date) <= {$top_days})";
    }
