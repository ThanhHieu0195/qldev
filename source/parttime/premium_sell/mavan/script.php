...
jQuery(document).ready(function($) {
      var oTable = $('#t_detail').dataTable( {
              "bProcessing": true,
              "bServerSide": true,
              "sAjaxSource": "ajaxserver.php?action=mavan",
              "aoColumnDefs": [
...
function show_form_edit(mavan, mota) {
  $('#edit-form input[name="mavan"]').val(mavan);
  $('#edit-form input[name="mavancu"]').val(mavan);
...
function show_form_del(mavan) {
    var r = confirm("Xóa phiếu: " + mavan);
    if (r==true) {
        window.location = "?action=del&mavan="+mavan;
    }
}
function checkvalueAdd() {
    var mavan = $('#add-form input[name = "mavan"]').val();
    var mota = $('#add-form input[name = "mota"]').val();
    check = true;
    if ( is_empty(mavan) ) {
        $('#add-form input[name = "mavan"]').parent().css('color', 'red').append('&nbsp;<span class="notify">is empty</span>');
        check = false;
    }
